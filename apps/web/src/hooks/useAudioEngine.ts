...
export function useAudioEngine() {
  ...
  function update(p: AudioParams) {
    ...
    const rainNorm = clamp(p.rainMm / 4);
    const precipNorm = clamp(p.precipitationMm / 4);
    const wetDayNorm = clamp(p.dailyRainMm / 20);
    ...
    const cutoffBase = scaleBetween(tempNorm * 0.7 + (1 - y) * 0.3, 0, 1, 500, 4800);
    const cutoff = Math.max(180, cutoffBase - 600 * rainNorm - 350 * wetDayNorm);
    const resonance = scaleBetween(humidityNorm, 0, 1, 0.5, 12);

    const rainTexture = Math.max(rainNorm, precipNorm * 0.85);
    const rainNoise = 0.015 + 0.14 * windNorm + 0.18 * rainTexture + 0.07 * wetDayNorm + 0.06 * pressure;
    const master = 0.04 + pressure * 0.08 + rainTexture * 0.03 + wetDayNorm * 0.02;

    const pointerLfoRate = 0.05 + (1 - y) * 0.9 + rainTexture * 0.18;
    const pointerLfoDepth = 0.01 + pressure * 0.04 + rainTexture * 0.05;

    const weatherWetness = clamp(humidityNorm * 0.6 + rainTexture * 0.3 + wetDayNorm * 0.35);
    const reverbWet = scaleBetween(weatherWetness, 0, 1, 0.08, 0.42);
    const dryLevel = 0.95 - reverbWet * 0.4 - wetDayNorm * 0.06;
    const reverbSend = scaleBetween(weatherWetness, 0, 1, 0.18, 0.56);

    const windLfoRate = 0.03 + windNorm * 0.45;
    const windLfoDepth = 0.02 + windNorm * 0.45 + rainTexture * 0.05;
    const windPanTarget = clamp(windPan * 0.75, -1, 1);
    const noiseBrightness = Math.max(1200, scaleBetween(tempNorm + windNorm * 0.4, 0, 1.4, 1800, 7000) - 800 * rainTexture);
